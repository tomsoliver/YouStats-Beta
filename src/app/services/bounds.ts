...
// From https://github.com/owid/owid-grapher/blob/master/charts/Bounds.ts
...
export class Bounds {
  ...
  static fromCorners(p1: Vector, p2: Vector) {
  ...
  static merge(boundsList: Bounds[]): Bounds {
    ...
    return Bounds.fromCorners(new Vector(x1, y1), new Vector(x2, y2));
  }
  ...
  static forText(
    str: string,
    {
      ...
      y = 0,
      fontSize = 16
    }: { x?: number; y?: number; fontSize?: number; fontFamily?: string } = {}
  ): Bounds {
    ...
    if (bounds) {
      if (bounds.x === x && bounds.y === y - bounds.height) {
        ...
        return bounds.extend({ x, y: y - bounds.height });
      }
    }
